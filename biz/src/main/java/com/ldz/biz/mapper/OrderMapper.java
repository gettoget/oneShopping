...
public interface OrderMapper extends Mapper<Order> , InsertListMapper<Order> {
    ...
    @Select( " select o.gmfs, o.cjsj,u.user_name userName,u.h_img hImg from Order_form o left join user u on u.id = o.user_id where o.pro_id = #{id} and o.user_id in ( select userid from order_list where yhlx = '0' and pro_id = #{id}) order by cjsj desc ")
